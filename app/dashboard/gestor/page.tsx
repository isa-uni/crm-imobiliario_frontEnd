...
import { useThemeColors, chartSeries, chartTooltipStyle, cssVar } from '@/hooks/useThemeColors'

const PIPELINE_COLORS: Record<string,string> = {
  lead: cssVar('chart-1'), oportunidade: cssVar('chart-3'), 'visita-agendada': cssVar('chart-2'),
  'visita-realizada': cssVar('chart-5'), pasta: cssVar('chart-4'), aprovado: cssVar('chart-6'), contrato: cssVar('chart-7'), descarte: cssVar('danger')
}

function KpiCard({ icon, label, value, sub, variacao }: { icon: React.ReactNode, label: string, value: string, sub?: string, variacao?: number }) {
  return (
    <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-5">
      <div className="flex items-center justify-between mb-4">
        <span className="w-10 h-10 rounded-xl bg-brand-soft text-brand-fg flex items-center justify-center">{icon}</span>
        {variacao !== undefined && variacao !== 0 && (
          <span className={`flex items-center gap-0.5 text-xs font-bold px-2 py-1 rounded-full ${variacao>0?'bg-success-bg text-success':'bg-danger-bg text-danger'}`}>
...
/** Anel de progresso circular — usado no resumo de metas para destacar o % atingido. */
function RadialProgress({ percent, size = 128, strokeWidth = 12, color = cssVar('brand-fg'), trackColor = cssVar('line'), children }:
  { percent: number; size?: number; strokeWidth?: number; color?: string; trackColor?: string; children?: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-surface">
      <header className="bg-sidebar border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent to-orange-600 flex items-center justify-center text-on-accent font-black text-sm shadow-btn">G</span>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <select value={filtros.corretorId} onChange={e=>setFiltros(f=>({...f, corretorId:e.target.value}))} className="px-3 py-2 rounded-btn border border-white/20 bg-card text-ink text-sm min-w-[160px]">
              <option value="">Todos corretores</option>
              {equipe.map(u=> <option key={u.id} value={u.id}>{u.nome}</option>)}
            </select>
            <select value={filtros.origem} onChange={e=>setFiltros(f=>({...f, origem:e.target.value}))} className="px-3 py-2 rounded-btn border border-white/20 bg-card text-ink text-sm min-w-[160px]">
              <option value="">Todas origens</option>
              {origemOptions.map(o=> <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
            <select value={filtros.status} onChange={e=>setFiltros(f=>({...f, status:e.target.value}))} className="px-3 py-2 rounded-btn border border-white/20 bg-card text-ink text-sm min-w-[160px]">
            ...
            {(filtros.corretorId || filtros.origem || filtros.status) && (
              <button onClick={()=>setFiltros(f=>({...f, corretorId:'', origem:'', status:''}))} className="px-3 py-2 rounded-btn bg-accent text-on-accent text-sm font-semibold">Limpar filtros</button>
            )}
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {/* KPIs */}
        <div>
        <h2 className="text-sm font-semibold text-muted uppercase tracking-wide mb-3">Visão geral do período</h2>
        ...
        {error && <div className="bg-warning-bg border border-warning-border text-warning px-4 py-3 rounded-card text-sm">{error}</div>}

        {/* Ranking corretores */}
        <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6 overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <h2 className="text-lg font-semibold text-ink flex items-center gap-2"><Award size={18} className="text-brand-fg"/> Corretores mais ativos</h2>
            <div className="flex gap-2">
              {(['negocios','leads','conversao'] as const).map(s=> (
                <button key={s} onClick={()=>setSortRanking(s)} className={`px-3 py-1.5 rounded-btn text-xs font-semibold border ${sortRanking===s?'bg-brand text-on-brand border-brand':'bg-card text-muted border-line hover:bg-surface'}`}>
                  ...
                  {rankingSorted.map((r,i)=> (
                    <tr key={r.corretorId} className="border-b border-line/60 hover:bg-surface/60">
                      <td className="py-3 font-medium text-ink flex items-center gap-2">
                        {i===0 && <span className="w-6 h-6 rounded-full bg-accent text-on-accent flex items-center justify-center text-xs font-black">1</span>}
                        {r.nome}
                      </td>
                      ...
                      <td className="text-center py-3">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${r.statusAtencao==='ok'?'bg-success-bg text-success':r.statusAtencao==='atencao'?'bg-warning-bg text-warning':'bg-danger-bg text-danger'}`}>
        ...
        <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6">
          <h2 className="text-lg font-semibold text-ink mb-4 flex items-center gap-2"><TrendingUp size={18} className="text-brand-fg"/> Pipeline comercial</h2>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                {d.pipeline.filter(p=>p.quantidade>0).map(p=> (
                  <div key={p.status} className="relative bg-card border border-line rounded-card p-4 pt-5 text-center overflow-hidden hover:shadow-card-lg transition-shadow">
                    <div className="absolute top-0 left-0 right-0 h-1" style={{ background: PIPELINE_COLORS[p.status]||cssVar('line') }} />
                    <p className="text-xs text-muted font-medium">{p.label}</p>
                    <p className="text-3xl font-bold text-ink mt-1.5 tracking-tight">{p.quantidade}</p>
                    <p className="text-xs text-muted mt-0.5">{p.percentual.toFixed(1)}%</p>
                    {p.valorPotencial>0 && <p className="text-xs text-success font-semibold mt-1.5">{fmtBRL(p.valorPotencial)}</p>}
                  </div>
                ))}
              </div>
              <div className="mt-6 h-10 flex rounded-full overflow-hidden border border-line">
                {d.pipeline.map(p=> (
                  <div key={p.status} style={{ width: `${p.percentual}%`, background: PIPELINE_COLORS[p.status]||cssVar('line') }} className="flex items-center justify-center text-[10px] font-bold text-on-chart" title={`${p.label}: ${p.quantidade}`}>
        ...
        {/* Metas */}
        <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6">
          <h2 className="text-lg font-semibold text-ink mb-2 flex items-center gap-2"><Target size={18} className="text-brand-fg"/> Meta da equipe — {format(new Date(filtros.inicio+'T12:00:00'), 'MMMM yyyy', {locale: ptBR})}</h2>
          {d.metas.metaContratosTotal===0 ? (
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 mb-6 p-4 bg-surface/60 rounded-card border border-line">
              <RadialProgress percent={d.metas.percentualContratos}>
                <span className="text-2xl font-bold text-ink">{d.metas.percentualContratos.toFixed(0)}%</span>
                <span className="text-[10px] text-muted uppercase tracking-wide">da meta</span>
              </RadialProgress>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 flex-1 w-full">
                <div className="bg-card rounded-card p-4 border border-line">
                  <p className="text-xs text-muted">Meta contratos</p>
                  <p className="text-2xl font-bold text-ink mt-1">{d.metas.metaContratosTotal}</p>
                </div>
                <div className="bg-brand rounded-card p-4">
                  <p className="text-xs text-sidebar-fg/80">Realizado</p>
                  <p className="text-2xl font-bold text-on-brand mt-1">{d.metas.realizadoContratos}</p>
                  <p className="text-xs text-accent font-medium mt-1">{d.metas.percentualContratos.toFixed(1)}% atingido</p>
                </div>
                <div className="bg-card rounded-card p-4 border border-line">
                  <p className="text-xs text-muted">Faltante</p>
                  <p className="text-2xl font-bold text-ink mt-1">{d.metas.faltanteContratos}</p>
                  <p className="text-xs text-muted mt-1">para bater a meta</p>
                </div>
              </div>
            </div>
          )}
                ...
                {d.metas.porCorretor.map(m=> (
                  <tr key={m.corretorId} className="border-b border-line/60">
                    ...
                    <td className="text-center py-2.5">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${m.status==='atingida'?'bg-success-bg text-success':m.status==='proxima'?'bg-warning-bg text-warning':m.status==='abaixo'?'bg-danger-bg text-danger':'bg-surface text-muted border border-line'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6">
            <h3 className="font-semibold text-ink mb-2 flex items-center gap-2"><Clock size={18} className="text-brand-fg"/> Tempo médio de conversão</h3>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={d.tempoMedio.porCorretor} layout="vertical">
                    ...
                    <XAxis type="number" tick={{fontSize:11, fill: cores.muted}}/>
                    <YAxis dataKey="nome" type="category" width={110} tick={{fontSize:11, fill: cores.muted}} tickFormatter={(v:string)=>v.length>14?v.slice(0,14)+'…':v}/>
                    <Tooltip formatter={(v:any)=>[`${Number(v).toFixed(1)} dias`, 'Média']} {...chartTooltipStyle(cores)}/>
                    <Bar dataKey="mediaDias" fill={cores["chart-1"]} radius={[0,6,6,0]} barSize={18}/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={d.tempoMedio.evolucao}>
                    <CartesianGrid strokeDasharray="3 3" stroke={cores.line}/>
                    <XAxis dataKey="mes" tick={{fontSize:11, fill: cores.muted}}/>
                    <YAxis tick={{fontSize:11, fill: cores.muted}}/>
                    <Tooltip formatter={(v:any)=>[`${Number(v).toFixed(1)} dias`, 'Média']} {...chartTooltipStyle(cores)}/>
                    <Line type="monotone" dataKey="mediaDias" stroke={cores.accent} strokeWidth={2} dot={{r:3}}/>
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6">
            <h3 className="font-semibold text-ink mb-4 flex items-center gap-2"><Tag size={18} className="text-brand-fg"/> Origem dos leads</h3>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <Tooltip formatter={(v:any,_n,props:any)=>[`${v} (${props.payload.percentual.toFixed(1)}%)`, 'Leads']} {...chartTooltipStyle(cores)}/>
                      <Legend iconType="circle" iconSize={8} wrapperStyle={{fontSize:12, color: cores.muted}}/>
                    </PieChart>
                  </ResponsiveContainer>
                </div>
          ...
          <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6">
            <h3 className="font-semibold text-ink mb-4 flex items-center gap-2"><Calendar size={18} className="text-brand-fg"/> Histórico de leads</h3>
            {d.historico.length===0 ? <p className="text-sm text-muted py-8 text-center">Sem histórico.</p> : (
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={d.historico}>
                    <CartesianGrid strokeDasharray="3 3" stroke={cores.line}/>
                    <XAxis dataKey="periodo" tick={{fontSize:11, fill: cores.muted}} tickFormatter={(v:string)=>v.slice(5)}/>
                    <YAxis tick={{fontSize:11, fill: cores.muted}}/>
                    <Tooltip {...chartTooltipStyle(cores)}/>
                    <Legend wrapperStyle={{color: cores.muted}}/>
                    <Area type="monotone" dataKey="recebidos" name="Recebidos" stroke={cores["chart-1"]} fill={cores["chart-1"]} fillOpacity={0.1}/>
                    <Area type="monotone" dataKey="contratos" name="Contratos" stroke={cores.success} fill={cores.success} fillOpacity={0.15}/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-card border border-line rounded-card shadow-card hover:shadow-card-lg transition-shadow p-6">
          ...
          {d.alertas.length===0 ? (
            ...
            <div className="grid gap-3">
              {d.alertas.map((a,i)=>(
                <div key={i} className={`px-4 py-3 rounded-card border flex items-start gap-3 ${a.severidade==='critical'?'bg-danger-bg border-danger-border text-danger':a.severidade==='warning'?'bg-warning-bg border-warning-border text-warning':'bg-surface border-line text-ink'}`}>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-success-bg border border-success-border rounded-card p-4">
              <p className="text-sm font-semibold text-success">Melhores desempenhos</p>
