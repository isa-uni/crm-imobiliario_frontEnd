      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <span className="w-12 h-12 rounded-xl bg-primary text-white flex items-center justify-center"><Bell size={24}/></span>
            <h1 className="text-3xl font-bold text-ink">Notificações</h1>
          </div>
          <button onClick={marcarTodas} className="text-sm text-primary hover:underline flex items-center gap-1"><Check size={16}/> Marcar todas como lidas</button>
        </div>
        <div className="space-y-3">
          {!Array.isArray(notifs) || notifs.length===0 ? <p className="text-center text-muted py-12">Nenhuma notificação</p> : notifs.map(n=> (
            <div key={n.id} className={`bg-white border rounded-card p-4 flex gap-3 ${n.lida ? 'opacity-60' : 'border-primary-200'}`}>
              <div className={`w-2 h-2 rounded-full mt-2 ${n.tipo==='CLIENTE_RECEBIDO' ? 'bg-green-500' : 'bg-amber-500'}`} />
              ...
              {!n.lida && <button onClick={()=>marcarLida(n.id)} className="text-xs text-primary font-semibold">Marcar lida</button>}
            </div>
          ))}
        </div>
      </div>
